...
{
	/*FILE *fsw;
	int iRet1 = fopen_s(&fsw, "dog2.yuv", "w+");
	if (iRet1 != 0) {
		std::cout << "open file failed ..." << std::endl;
		exit(-1);
	}

	cv::Mat src_jpg = cv::imread("dog.jpg");
	cv::Mat dst_yuv;
	cv::cvtColor( src_jpg, dst_yuv, cv::COLOR_BGR2YUV_I420 );
	cv::imshow("YUV", dst_yuv);
	std::cout << "yuv size = " << dst_yuv.size() << std::endl;

	int size = dst_yuv.size().width * dst_yuv.size().height;
	std::cout << "yuv size = " << size << std::endl;

	unsigned char *buffer = new unsigned char[ size ];
	memcpy( buffer, dst_yuv.data, size );

	int wlen = fwrite( buffer, sizeof( unsigned char ), size, fsw );
	...
	FILE *fs;
	int iRet = fopen_s(&fs, "test.yuv", "rb");
	if (iRet != 0) {
		std::cout << "open file failed ..." << std::endl;
		exit(-1);
	}
	fseek(fs, 0, SEEK_END);
	int size = ftell(fs);//返回给定流stream的当前文件位置。一般作用是获取文件大小，以定义buf大小
	std::cout << "file size = " << size << std::endl;

	int buffSize = 1280 * 720 * 3 / 2;
	fseek(fs, 0, SEEK_SET);

	unsigned char *buff = new unsigned char[buffSize];
	memset( buff, 0, buffSize );

	int rlen = fread( (void *)buff, sizeof(unsigned char), buffSize, fs);

	std::cout << "buff size = "<< rlen << std::endl;


	cv::Mat src_yuv;
	src_yuv.create(720 * 3 / 2, 1280, CV_8UC1);
	memcpy(src_yuv.data, buff, buffSize * sizeof(unsigned char));
	...
	sendPieces( sock_fd, addr_client, len2, basedStr);
